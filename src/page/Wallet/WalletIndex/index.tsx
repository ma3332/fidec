...
const WalletIndexPage = ({mnemonic, setCheckImportMnemonic, index}: Props) => { 

  const dispatch = useDispatch();

  const web3 = new Web3("https://goerli.infura.io/v3/eb4e3ec2fd9c4b92ae0142d44172e91e")
  
  const [listAccount, setListAccount] = useState([]);
  const [privateKey, setPrivateKey] = useState([]);
  const [accountOption, setAccountOption] = useState({privateKey: "0x", publicKey: "0x", balance: "0"}); 
    ...
    <div className="walletIndex--page">
      ...
      <div className="main">
        <div className="wallet--content">
          ...
          <Radio.Group className="displayFlexColumn indexContainer" onChange={(e)=>setPrivateKey(e.target.value)}>
            {listAccount?.map(
               (item, index) => { 
                return (
                  <Radio key={index} value={item.privateKey}>
                    <div className="flex">
                      <p>{index}</p> 
                      <p>{item.publicKey.slice(0, 8) +"..."+ item.publicKey.slice(-6)}</p>
        ...
        <div className="inputIndexContainer">
          <div className="item">
            <Input placeholder="Your Index" onChange={e=>createAccountOption(Number(e.target.value))}/>
          </div>
          <div className="item">
            <p>{accountOption.publicKey.slice(0, 5)+"..."+accountOption.publicKey.slice(0, -3)}</p>
          </div>
          <div className="item">
            <p>{accountOption.balance.slice(0, 8)}</p>
          </div>
        </div>
        <button className="btn--gray--default" onClick={()=> importAccountMnemonic()}>Import</button>
      </div>
      <BackToHomeText />
      <Navbar /> 
    </div> 
